...
import { getShapeName } from '../clazz';
...
export default function(G6){
  G6.registerBehavior('dragPanelItemAddNode', {
    ...
    onMouseUp(e){
      if (this.graph.get('addNodeDragging')) {
        // edit this function from https://github.com/guozhaolong/wfd-vue/issues/67
        ...
        const subProcessNode = this.graph.find('node', (node) => {
          if (node.get('model')) {
            ...
            if (clazz === 'subProcess') {
              ...
              return x > bbox.minX && bbox.maxX > x
                && y > bbox.minY && bbox.maxY > y;
            }
          } else {
            return false;
          }
